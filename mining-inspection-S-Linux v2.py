...
def processing_report(datas):
    ...
    filename = pr_path + '/xls/Write_check_server_report_' + data[0]['pc_name'] + '_' + data[0]['pc_ip'] + '_' + str(
        uuid.uuid4()) + '.xls'
    book = xlwt.Workbook()
    writetext = ['主机名', 'IP地址', '系统类型', 'CPU', '内存', '异常端口', '带宽占用', '系统盘占用', '本地运行中进程名称', '本地运行中进程路径','本地运行中进程HASH', '外连IP', '异常项']
    try:
    ...
    for i in range(1, len(data[0]['process_hash'][0][0])):
        sheet.write(9, i, data[0]['process_hash'][0][0][i - 1][0].replace('\\\\','\\'))
